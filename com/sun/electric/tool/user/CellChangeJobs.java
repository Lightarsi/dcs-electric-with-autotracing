...
public class CellChangeJobs {
    ...
    public static class DeleteCell extends Job {
        ...
        public boolean doIt() throws JobException {
            ...
            if (cell.isInUse("delete", false, true)) {
                return false;
            }
            cell.kill();
    ...
    public static class GraphCells extends Job {
        ...
        public boolean doIt() throws JobException {
            ...
            if (graphCell.getNumVersions() > 1) {
                System.out.println("Creating new version of cell: " + graphCell.getName());
            } else {
            ...
            for (Cell cell : graphNodes.keySet()) {
                ...
                if (xval[cgn.depth] > maxWidth) {
                    maxWidth = xval[cgn.depth];
                }
                cgn.y = cgn.depth;
                cgn.yoff = 0;
            }
        ...
        /**
         * Method to find the main cell that "cell" is associated with in the
         * graph. This code is essentially the same as "contentscell()" except
         * that any original type is allowed.
         *
    ...
    public static class GraphLibraries extends Job {
        ...
        public boolean doIt() throws JobException {
            ...
            for (Iterator<Library> it = Library.getLibraries(); it.hasNext();) {
                ...
                for (Iterator<Cell> cIt = lib.getCells(); cIt.hasNext();) {
                    ...
                    for (Iterator<NodeInst> nIt = cell.getNodes(); nIt.hasNext();) {
                        ...
                        if (subLibs == null) {
                            libraryDependencies.put(lib, subLibs = new HashSet<Library>());
                        }
                        subLibs.add(subLib);
            ...
            for (GraphArc ga : allArcs) {
                ...
                if (ga.doubleHeaded) {
                    color = EGraphics.LRED;
                }
                if (trueCgn.topLibrary) {
                    color = EGraphics.BLUE;
                } else if (trueSubCgn.leafLibrary) {
                    color = EGraphics.GREEN;
                }
                ai.newVar(Artwork.ART_COLOR, new Integer(color), ep);
    ...
    /**
     * This class implement the command to extract the contents of cell
     * instances.
     */
    ...
    private static void extractOneLevel(Cell cell, ExpansionState expansionState, Set<NodeInst> expandedNodes,
        ...
        for (Iterator<NodeInst> it = subCell.getNodes(); it.hasNext();) {
            ...
            if (Generic.isEssentialBnd(ni) && hasEssentialBounds) {
                continue;
            }
...
//					newAi.setHeadExtended(a.isTailExtended()); newAi.setTailExtended(a.isHeadExtended());
    ...
    public static class DuplicateCell extends Job {
        ...
        public boolean doIt() throws JobException {
            ...
            for (Cell otherCell : cell.getCellsInGroup()) {
                ...
                if (!entireGroup && !(cell.isSchematic() && otherCell.isIcon())) {
                    continue;
                }
    ...
    /**
     * Copy expanded status in client database after some cells were copied or
     * moved in server database
     *
     ...
     * @param allRelatedViews true to copy all related views (schematic cell
     * with layout, etc.) If false, only schematic/icon relations are copied.
     * @param copySubCells true to recursively copy sub-cells. If true,
     * "useExisting" must be true.
     * @param useExisting true to use any existing cells in the destination
     * library instead of creating a cross-library reference. False to copy
     * everything needed.
     ...
     * @param verbose true to display extra information.
     * @param move true to move instead of copy.
     * @param subDescript a String describing the nature of this copy (empty
     * string initially).
     * @param schematicRelatedView true to copy a schematic related view.
     * Typically this is true, meaning that if copying an icon, also copy the
     * schematic. If already copying the example icon, this is set to false so
     * that we don't get into a loop.
     * @param allRelatedViews true to copy all related views (schematic cell
     * with layout, etc.) If false, only schematic/icon relations are copied.
     * @param allRelatedViewsThisLevel true to copy related views for this level
     * of invocation only (but further recursion will use "allRelatedViews").
     * @param copySubCells true to recursively copy sub-cells. If true,
     * "useExisting" must be true.
     * @param useExisting true to use any existing cells in the destination
     * library instead of creating a cross-library reference. False to copy
     * everything needed.
     * @param existing a map that disambiguates cell names when they clash in
     * different original libraries. The main key is an old cell name, and the
     * value for that key is a map of library names to new cell names. So, for
     * example, if libraries "A" and "B" both have a cell called "X", then
     * existing.get("X").get("A") is "X" but existing.get(X").get("B") is "X_1"
     * which disambiguates the cell names in the destination library.
     * @param idMapper mapper which handles renamed cells
     * @param newCells mapper which handles both copied and renamed cells
     * @param deleteThese a list of Cells to delete when all is done (if moving
     * instead of copying)
     * @param ep EditingPreferences
     */
    private static Cell copyRecursively(Cell fromCell, Library toLib, boolean verbose, boolean move, String subDescript,
        ...
        if (copySubCells || fromCell.isSchematic()) {
            ...
            while (found) {
                ...
                for (Iterator<NodeInst> it = fromCell.getNodes(); it.hasNext();) {
                    NodeInst ni = it.next();
                    if (!copySubCells && !ni.isIconOfParent()) {
                        continue;
                    }
                    if (!ni.isCellInstance()) {
                        continue;
                    ...
                    if (cell.getLibrary() == toLib) {
                        continue;
                    }

                    // see if the cell is already there
                    if (inDestLib(cell, existing, toLib) != null) {
                        continue;
                    }

                    // do not copy subcell if it exists already (and was not copied by this operation)
                    if (useExisting && !copySubCells) {
                        if (toLib.findNodeProto(cell.noLibDescribe()) != null) {
                            continue;
                        }
                    }
            ...
            if (toView == View.ICON && schematicRelatedView /*&& move*/) {
                ...
                while (found) {
                    ...
                    for (Cell np : fromCell.getCellsInGroup()) {
                        ...
                        // see if the cell is already there
                        if (inDestLib(np, existing, toLib) != null) {
                            continue;
                        }
        ...
        if (newName == null) {
            for (int i = 0; i < 1000; i++) {
                ...
                if (!libToNameMap.values().contains(newName)) {
                    break;
                }
            }
...
//			fromCell.kill();
     ...
     * @return a Cell from the destination library that matches the Cell being
     * searched (null if none).
     */
