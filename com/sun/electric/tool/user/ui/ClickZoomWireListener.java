 ...
 * <p>
 * The Mouse Wheel handles panning
 *
 * User: gainsley
 */
public class ClickZoomWireListener
    ...
    /* cancel move delay in milliseconds */
    private long zoomInDelayMillis;
    /* zoom in delay in milliseconds */
    ...
    /* current mouse pressed coordinates in screen space */
    private int lastX, lastY;
    /* last mouse pressed coordinates in screen space (for panning) */
    private Cell startCell;
    private double dbMoveStartX, dbMoveStartY;
    /* left mouse pressed coordinates for move in database space */
    private double lastdbMouseX, lastdbMouseY;
    /* last location of mouse */
    private Mode modeLeft = Mode.none;
    /* left mouse button context mode */
    private Mode modeRight = Mode.none;
    /* right mouse button context mode */
    private Mode modeMiddle = Mode.none;
    /* middle mouse button context mode */
    private boolean specialSelect = false;
    /* if hard-to-select objects are to be selected */
    ...
    /**
     * Returns state of 'stickyMove'. If sticky move is true, after the user
     * clicks and drags to move an object, the user can release the mouse button
     * and the object will continue to move with the mouse. Clicking the select
     * mouse key again will place the object. If sticky move is false, the user
     * must hold and drag to move the object. Letting go of the select mouse key
     * will place the object. This is the C-Electric style.
     *
    ...
    /**
     * Returns state of 'stickyWiring'. If sticky wiring is true, after the user
     * clicks and drags to draw a wire, the user can release the mouse button
     * and the UI will remain in wire-draw mode. Click the mouse button again
     * will draw the wire. If sticky wiring is false, the user must hold and
     * drag to draw the tentative wire, and the wire gets drawn when the user
     * releases the mouse button. This is C-Electric style.
     *
    ...
    /**
     * Return the last known location of the mouse. Note that these are screen
     * coordinates, and are in the coordinate system of the current
     * container(?).
     *
    ...
    /**
     * See if event is a right mouse click. Platform independent. One-button
     * macintosh: Command + click == right mouse click.
     */
    ...
    public void mousePressed(MouseEvent evt) {
        ...
        if (debug) {
            System.out.println("  [" + modeLeft + "," + modeRight + "," + modeMiddle + "] " + evt.paramString());
        }
 ...
 /* if no modifiers, do "get info" */
                ...
                if (User.isRoutingMode()) {
                    ...
                    for (Iterator<Geometric> it = cell.searchIterator(searchArea); it.hasNext();) {
                        ...
                        if (gridWid == 0) {
                            gridWid = DBMath.lambdaToSizeGrid(1);
                        }
                ...
                if (User.isRoutingMode() && dragArc != null) {
                    ...
                    if (dragLandingHighlights == null) {
                        ...
                        for (PortInst pi : dragArcPossiblePorts) {
                            if (pi == dragArcPort || pi == dragArcOtherPort) {
                                continue;
                            }
                            EPoint ctr = pi.getCenter();
                            if (ctr.getX() < bounds.getMinX() || ctr.getX() > bounds.getMaxX()
                                    || ctr.getY() < bounds.getMinY() || ctr.getY() > bounds.getMaxY()) {
                                continue;
                            }
...
//	                		        }
                    ...
                    if (moveDelta != null) {
                        highlighter.remove(moveDelta);
                    }
                    if (moveDRC != null) {
                        highlighter.remove(moveDRC);
                    }
                    ...
                    if (ws.validSpacing()) {
                        ...
                        if (ws.getOneLayer() != ws.getOtherLayer()) {
                            message += " to " + ws.getOtherLayer().getName();
                        }
    ...
    /**
     * Class to detect the worst design-rule violation while moving a node or
     * arc.
     */
    private static class WorstSpacing {
        ...
        /**
         * Method to update this WorstSpacing object according to a polygon on a
         * Geometric.
         *
         ...
         * @param delta the offset that is being applied to the polygon (how
         * much it has been dragged).
            ...
            for (Iterator<Geometric> it = self.getParent().searchIterator(searchBounds); it.hasNext();) {
                ...
                if (neighbor instanceof NodeInst) {
                    ...
                    for (int i = 0; i < otherPolys.length; i++) {
                        ...
                        if (otherLay.getFunction().isSubstrate()) {
                            continue;
                        }
                        if (lay.getTechnology() != otherLay.getTechnology()) {
                            continue;
                        }
                        ...
                        if (valid && sep - dist >= worstViolation) {
                            continue;
                        }
        ...
        /**
         * Method to examine load the "worstFrom" and "worstTo" field variables
         * with the closest two points on two polygons.
         *
    ...
    public void mouseReleased(MouseEvent evt) {
        ...
        if (evt.getSource() instanceof EditWindow) {
            ...
            if (isRightMouse(evt)) {
                ...
                if (modeRight == Mode.drawBox || modeRight == Mode.zoomBox || modeRight == Mode.zoomBoxSingleShot) {
                    ...
                    if (Math.abs(wnd.getStartDrag().getX() - wnd.getEndDrag().getX()) > 10
                            || Math.abs(wnd.getStartDrag().getY() - wnd.getEndDrag().getY()) > 10) {
                        onePoint = false;
                    }
            ...
            if (isLeftMouse(evt)) {
                ...
                if (debug) {
                    System.out.println("Time diff between click->release is: " + (curTime - leftMousePressedTimeStamp));
                }
                if (modeLeft == Mode.move || modeLeft == Mode.stickyMove) {
    ...
    private void mouseOver(Point2D dbMouse, EditWindow wnd) {
        ...
        if (found == null) {
            ...
            if (mouseOld.size() != 1 || !found.sameThing(mouseOld.get(0), true)) {
                changed = true;
            }
        }
...
//            wnd.getWindowFrame().getFrame().getStatusBar().highlightChanged(highlighter);
    ...
    /**
     * Mouse Wheel Events are used for panning Wheel Forward: scroll up Wheel
     * Back: scroll down SHIFT + Wheel Forward: scroll right SHIFT + Wheel Back:
     * scroll left
     *
    ...
    public void wireTo(int layerNumber) {
        ...
        for (Iterator<ArcProto> it = tech.getArcs(); it.hasNext();) {
            ap = it.next();
            if (ap.isNotUsed()) {
                continue;               // ignore arcs that aren't used
            }
            switch (layerNumber) {
                case 0: {
                    if (ap.getFunction() == ArcProto.Function.POLY1) {
                        found = true;
                    }
                    break;
                }
                case 1: {
                    if (ap.getFunction() == ArcProto.Function.METAL1) {
                        found = true;
                    }
                    break;
                }
                case 2: {
                    if (ap.getFunction() == ArcProto.Function.METAL2
                            || ap.getFunction() == ArcProto.Function.BUS) {
                        found = true;
                    }
                    break;
                }
                case 3: {
                    if (ap.getFunction() == ArcProto.Function.METAL3) {
                        found = true;
                    }
                    break;
                }
                case 4: {
                    if (ap.getFunction() == ArcProto.Function.METAL4) {
                        found = true;
                    }
                    break;
                }
                case 5: {
                    if (ap.getFunction() == ArcProto.Function.METAL5) {
                        found = true;
                    }
                    break;
                }
                case 6: {
                    if (ap.getFunction() == ArcProto.Function.METAL6) {
                        found = true;
                    }
                    break;
                }
                case 7: {
                    if (ap.getFunction() == ArcProto.Function.METAL7) {
                        found = true;
                    }
                    break;
                }
                case 8: {
                    if (ap.getFunction() == ArcProto.Function.METAL8) {
                        found = true;
                    }
                    break;
                }
                case 9: {
                    if (ap.getFunction() == ArcProto.Function.METAL9) {
                        found = true;
                    }
                    break;
                }
        ...
        if (highlighter.getNumHighlights() == 1 && cell != null) {
            ...
            if (obj instanceof PortInst) {
                ...
                for (ArcProto ap : connArcs) {
                    ...
                    if (!ap.getFunction().isDiffusion() && !ap.getFunction().isPoly() && !ap.getFunction().isMetal()) {
                        continue;
                    }
                    if (sourceAp == null) {
                        sourceAp = ap;
                    ...
                    if (down && ap.getFunction().isPoly() && sourceAp.getFunction().isMetal()) {
                        sourceAp = ap;
                        continue; // poly is lower than metal
                    }
                    ...
                    if (sourceAp.getFunction().isDiffusion() || ap.getFunction().isDiffusion()) {
                        continue;
                    }
...
//                      found = true; break;
//                  }
                    if (metal && destAp.getFunction() == ArcProto.Function.getMetal(level)) {
                        found = true;
                        break;
                    }
    ...
    // ********************************* Routing Mode ********************************
    /**
     * Method to recursively follow one end of an ArcInst and gather connected
     * PortInsts and ArcInsts. Adds likely PortInsts to the
     * "dragArcPossiblePorts" set. Adds arcs on the path to the
     * "dragArcSeenArcs" set.
     *
    ...
    private void followEnd(PortInst end, ArcInst ai) {
        if (dragArcSeenArcs.contains(ai)) {
            return;
        }
    ...
    private void clearArcDragHighlighting(Highlighter highlighter) {
        ...
        if (dragLandingHighlights != null) {
            for (Highlight h : dragLandingHighlights) {
