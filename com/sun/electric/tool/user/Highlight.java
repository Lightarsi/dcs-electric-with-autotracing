 ...
 * the user's attention is being called (an ElectricObject, some text, a region,
 * etc) and enough information to render the highlighting (boldness, etc) on any
 * given window. It is not specific to any given EditWindow.
 */
public abstract class Highlight implements Cloneable {
     ...
     */
    public static final BasicStroke solidLine = new BasicStroke(0);
    /**
     * for drawing dotted lines
     */
    public static final BasicStroke dottedLine = new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[]{1}, 0);
    /**
     * for drawing dashed lines
     */
    public static final BasicStroke dashedLine = new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10, new float[]{10}, 0);
    /**
     * for drawing dashed lines
     */
    public static final BasicStroke boldLine = new BasicStroke(3);
     ...
     * The two possibilities are (1) text on invisible pins (2) export names,
     * when the option to move exports with their labels is requested.
     *
    ...
    /**
     * the highlight pattern will repeat itself rotationally every
     * PULSATE_ROTATE_PERIOD milliseconds
     */
    ...
    /**
     * the number of "segments" in each stripe; increasing this number slows
     * down rendering
     */
    ...
    /**
     * highlight objects that are electrically connected to this object unless
     * specified not to. HighlightConnected is set to false by addNetwork when
     * it figures out what's connected and adds them manually. Because they are
     * added in addNetwork, we shouldn't try and add connected objects here.
     *
     * @param g2
     * @param wnd
     */
    ...
    /**
     * Method to return the Geometric object that is in this Highlight. If the
     * highlight is a PortInst, an Export, or annotation text, its base NodeInst
     * is returned.
     *
     * @return the Geometric object that is in this Highlight. Returns null if
     * this Highlight is not on a Geometric.
     */
    ...
    /**
     * Method to return a List of all highlighted NodeInsts. Return a list with
     * the highlighted NodeInsts.
     */
    ...
    /**
     * Method to return a List of all highlighted ArcInsts. Return a list with
     * the highlighted ArcInsts.
     */
    ...
    /**
     * Method to return a set of the currently selected networks. Return a set
     * of the currently selected networks. If there are no selected networks,
     * the list is empty.
     */
     ...
     * @param unique true to request that the text objects be unique, and not
     * attached to another object that is highlighted. For example, if a node
     * and an export on that node are selected, the export text will not be
     * included if "unique" is true. Return a list with the Highlight objects
     * that point to text.
     */
     ...
     * @return the bounds of the highlighted objects (null if nothing is
     * highlighted).
     */
    ...
    /**
     * Method to return the ElectricObject associated with this Highlight
     * object.
     *
     ...
     * @return (possible updated) this Highlight if the point is over this
     * Highlight, null otherwise
     */
    ...
    /**
     * Method to get a list of Arcs and Nodes from a Highlight list. If a port
     * is found, it adds the node instance associated to.
     *
    ...
    /**
     * Method to load an array of counts with the number of highlighted objects
     * in a list. arc = 0, node = 1, export = 2, text = 3, graphics = 4
     *
     * @param list the list of highlighted objects.
    ...
    public static void drawOutlineFromPoints(EditWindow wnd, Graphics g, Point2D[] points, long offX, long offY,
        ...
        for (int i = 1; i < points.length; i++) {
            ...
            if (DBMath.doublesEqual(p.getX(), firstP.getX())
                    && DBMath.doublesEqual(p.getY(), firstP.getY())) {
                continue;
            }
            onePoint = false;
            break;
    ...
    /**
     * Implementing clipping here speeds things up a lot if there are many large
     * highlights off-screen
     */
...
//        }
    ...
    /**
     * General purpose function to sort Highlight objects based on their getInfo
     * output
     */
    ...
    /**
     * A Highlight which calls the user's attention to a Point2D and includes a
     * text message.
     */
...
class HighlightLine extends Highlight {
     ...
     */
    protected final Point2D start, end, center;
    /**
     * The highlighted line is thick.
     */
    protected final boolean thickLine;
    /**
     * The highlighted line can only draw in this window
     */
    ...
    public void showInternalHighlight(EditWindow wnd, Graphics g, long highOffX, long highOffY,
            boolean onlyHighlight) {
        if (onlyThis != null && onlyThis.get() != wnd.getWindowFrame()) {
            return;
        }
...
class HighlightObject extends Highlight {
    ...
    /**
     * The highlighted generic object
     */
...
class HighlightArea extends Highlight {
    ...
    void getHighlightedEObjs(Highlighter highlighter, List<Geometric> list, boolean wantNodes, boolean wantArcs) {
        ...
        for (Highlight ah : inArea) {
            if (!(ah instanceof HighlightEOBJ)) {
                continue;
            }
...
class HighlightEOBJ extends Highlight {
    ...
    /**
     * For Highlighted networks, this prevents excess highlights
     */
    private final boolean highlightConnected;
    /**
     * The highlighted outline point (only for NodeInst).
     */
                ...
                if (points != null) {
                    ...
                    if (points[point] != null) {
                        ...
                        if (prevPoint >= 0 && points[prevPoint] != null) {
                            ...
                            if (prevPt.getX() == thisPt.getX() && prevPt.getY() == thisPt.getY()) {
                                prevPoint = -1;
                            } else {
                        ...
                        if (nextPoint >= 0 && points[nextPoint] != null) {
                            ...
                            if (nextPt.getX() == thisPt.getX() && nextPt.getY() == thisPt.getY()) {
                                nextPoint = -1;
                            } else {
            ...
            if (pp != null) {
                ...
                if (ni.isCellInstance() && (g instanceof Graphics2D)) {
                    ...
                    if (wired) {
                        ...
                        ScreenPoint scrP = wnd.databaseToScreen(poly.getCenterX(), poly.getCenterY());
                        if (v2!=null) {
                             // if 5400TP035 key => translate down a little bit 
                            ((Graphics2D) g).drawGlyphVector(v2, (float) scrP.getX() + offX, (float) scrP.getY() + offY - 5f);
                        } else {
                            ((Graphics2D) g).drawGlyphVector(v, (float) scrP.getX() + offX, (float) scrP.getY() + offY);
                        }
    ...
    @Override
    void showHighlightsConnected(Graphics2D g2, EditWindow wnd) {
        if (!isValid() || !(eobj instanceof PortInst) || !highlightConnected) {
            return;
        }
    ...
    public Geometric getGeometric() {
        ...
        if (eobj instanceof PortInst) {
            retVal = ((PortInst) eobj).getNodeInst();
        } else if (eobj instanceof Geometric) {
            retVal = (Geometric) eobj;
        }
        return retVal;
    }
        ...
        if (eobj instanceof Geometric) {
            ...
            for (Highlight got : gotAll) {
                ...
                for (Highlight alreadyDone : highlighter.getHighlights()) {
                    if (!(alreadyDone instanceof HighlightEOBJ)) {
                        continue;
                    }
...
/**
 * A Highlight which calls the user's attention to an ElectricObject which
 * happens to be a piece of text.
 */
    ...
    public Geometric getGeometric() {
        if (DisplayedText.objectMovesWithText(eobj, varKey, User.isMoveNodeWithExport())) {
            if (eobj instanceof Export) {
                return ((Export) eobj).getOriginalPort().getNodeInst();
            }
            if (eobj instanceof Geometric) {
        ...
        if (unique) {
            ...
            if (eobj != null) {
                ...
                for (Highlight oH : getHighlights) {
                    if (!(oH instanceof HighlightEOBJ)) {
                        continue;
                    }
    ...
    public String describe() {
        ...
        if (varKey != null && eobj != null) {
            if (varKey == NodeInst.NODE_NAME) {
                ...
                if (var != null) {
                    description = eobj.getFullDescription(var);
                }
            }
        }
        return description;
