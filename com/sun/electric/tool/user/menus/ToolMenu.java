...
import com.sun.electric.tool.autotracing.*;
...
public class ToolMenu {
        ...
        return new EMenu("_Tools",
                ...
                new EMenu("Simulation (_Spice)",
                    ...
                    public void run() {
                        ...
                        new EMenuItem("Export Autotracing Scheme") {
                    public void run() {
                        FileMenu.exportCommand();
                    }
                },
    ...
    private static ArrayList<Pair<ArcInst, String>> setName() {
        ...
        /*List<ArcInst> theArcs = new ArrayList<>();
        theArcs = highlighter.getHighlightedArcs();*/
    ...
    public static void MakeTrace() {
        Autotracing.makeTrace();
    }

    /**
     * Method to use Expert System for precision schemes.
     */
    public static void getAdvice() {
        Autotracing.getAdvice();
    }

    /**
     * Method to invoke the Bean Shell on a script file.
     */
    public static void javaBshDo(String ScriptName) {
        if (ScriptName != null) {
            // start a job to run the script
            EvalJavaBsh.runScript(ScriptName);
        }
    }

    /**
     * Method to check for updates and update if available
     */
    private static Document loadTestDocument(URLConnection connection) throws Exception {
    ...
    public static void checkUpdates(boolean fromMenu) throws Exception {
        if (!fromMenu) {
            Properties prop = new Properties();
            InputStream input = null;
            try {
                File file = new File("C:\\CYGELENG\\electric\\properties.ini");
                input = new FileInputStream(file);
                // load a properties file
                prop.load(input);
                if (prop.getProperty("wantUpdate").equals("no")) {
                    return;
                }
        ...
        URL url = new URL("http://dcsoyuz.com/files/content/CUscr/Update.xml");
        ...
        if (connection.getContentLengthLong() == (-1)) {
            if (fromMenu == false) {
                ...
                JOptionPane.showMessageDialog(frame,
                        "Connection failed. Please, contact our support team.");
                frame.dispose();
            }
            return;
        }
        Document doc = null;
        try {
            doc = loadTestDocument(connection);
        } catch (ParserConfigurationException pce) {
            pce.printStackTrace();
            JOptionPane.showMessageDialog(frame,
                    "Document can not be parsed. Please, contact our support team.");
            frame.dispose();
            return;
        }
        NodeList versionNodeList = doc.getElementsByTagName("versio");
        String conti;
        try {
            conti = versionNodeList.item(0).getTextContent();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(frame,
                    "Version check failed. Please, contact our support team.");
            frame.dispose();
            return;
        }
        File versionFile = new File("C:\\CYGELENG\\electric\\version.ini");
        FileReader versionReader = new FileReader(versionFile);
        BufferedReader versionBufReader = new BufferedReader(versionReader);
        String version = "1.0";
        String line = "";
        while ((line = versionBufReader.readLine()) != null) {
            try {
        ...
        if (!conti.equals(version)) {
            System.out.println("Your version is " + version + ". Update is required");
            if (!fromMenu) {
                Object[] options = {"Update", "Skip", "Remind later"};
                int res = -2;
                res = JOptionPane.showOptionDialog(null, "Would you like to update?", "Update",
                ...
                if (res == JOptionPane.YES_OPTION) {
                    // nothing here (it means update).
                } else if (res == JOptionPane.NO_OPTION) {
                    try {
                        Properties properties = new Properties();
                        properties.setProperty("wantUpdate", "no");
                        File file = new File("C:\\CYGELENG\\electric\\properties.ini");
                ...
                Object[] options = {"Update", "Don't Update",};
                int res = -2;
                res = JOptionPane.showOptionDialog(null, "Would you like to update?", "Update",
            ...
            System.out.println("Your version is " + version + ". Update is not required");
            frame.dispose();
            return;
        }

        NodeList actionNodeList = doc.getElementsByTagName("v" + version);
        NodeList childActionNodeList;

        if (actionNodeList.getLength() > 0) {
            childActionNodeList = actionNodeList.item(0).getChildNodes();
        } else {
            System.out.println("Parsing operation failed. Please, contact our support team.");
            JOptionPane.showMessageDialog(frame,
                    "Document can not be parsed. Please, contact our support team.");
            frame.dispose();
            return;
        ...
        try {
            for (int i = 0; i < childActionNodeList.getLength(); i++) {
                if (childActionNodeList.item(i).getNodeName().equals("copy")) {
                    try {
                        java.nio.file.Path source = java.nio.file.Paths.get(childActionNodeList.item(i).getChildNodes().item(0).getTextContent());
                        java.nio.file.Path dest = java.nio.file.Paths.get(childActionNodeList.item(i).getChildNodes().item(1).getTextContent());
                ...
                } else if (childActionNodeList.item(i).getNodeName().equals("rename")) {
                    boolean success = false;
                    File file = null;
                    File file2 = null;
                    File file3 = null;
                    try {
                        file = new File(childActionNodeList.item(i).getChildNodes().item(0).getTextContent());
                        file2 = new File(childActionNodeList.item(i).getChildNodes().item(1).getTextContent());
                        success = file.renameTo(file2);
                        if (!success) {
                            file2 = new File(childActionNodeList.item(i).getChildNodes().item(1).getTextContent());
                            String filePath = childActionNodeList.item(i).getChildNodes().item(1).getTextContent();
                            String[] parts = filePath.split("\\.");
                            if (parts.length != 2) {
                                JOptionPane.showMessageDialog(frame,
                                        "Update file is broken." + parts.length);
                                frame.dispose();
                                return;
                            }
                            file3 = new File(parts[0] + "_old." + parts[1]);
                            file2.renameTo(file3);
                            try {
                                success = file.renameTo(file2);
                                if (!success) {
                                    JOptionPane.showMessageDialog(frame,
                                            "Please, delete .old files from your project.");
                                    frame.dispose();
                                    return;
                                }
                            } catch (Exception e1) {
                ...
                } else if (childActionNodeList.item(i).getNodeName().equals("delete")) {
                    try {
                        File fileForDel = new File(childActionNodeList.item(i).getChildNodes().item(0).getTextContent());
                ...
                } else if (childActionNodeList.item(i).getNodeName().equals("download")) {
                    try {
                        // качаем файл с помощью NIO
                        downloadUsingNIO(childActionNodeList.item(i).getChildNodes().item(0).getTextContent(),
                                childActionNodeList.item(i).getChildNodes().item(1).getTextContent());
                    } catch (IOException e) {
                        e.printStackTrace();
                        System.out.println("Download failed.");
                    }
                }
                //final int prog = Math.round((100/(childActionNodeList.getLength()))*i);
            }
        } catch (Exception e) {
            System.out.println("CRDD operation failed. Please, contact our support team.");
            frame.dispose();
            return;
        }
        PrintWriter out = new PrintWriter(versionFile.getAbsoluteFile());
        String text = "version = " + conti;
        try {
            out.print(text);
        } finally {
            out.close();
        ...
        frame.dispose();
        JOptionPane.showMessageDialog(frame,
                "Project updated. Your new version is " + conti + ".");

        try {
            Properties properties = new Properties();
            properties.setProperty("wantUpdate", "yes");
            File file = new File("C:\\CYGELENG\\electric\\properties.ini");
            FileOutputStream fileOut = new FileOutputStream(file);
            properties.store(fileOut, "Properties");
            fileOut.close();
