...
public class CircuitChangeJobs {
    ...
    public static class RotateSelected extends Job {
         ...
         * @param mirror whether or not to mirror. if true, amount is ignored,
         * and mirrorH is used.
         * @param mirrorH if true, mirror horizontally (flip over X-axis),
         * otherwise mirror vertically (flip over Y-axis). Ignored if mirror is
         * false.
         */
...
//                if (Generic.isSpecialGenericNode(ni))
    ...
    public static class RotateText extends Job {
         ...
         * @param objs the objects (nodes, arcs, exports) that hold text to
         * rotate.
        ...
        public boolean doIt() throws JobException {
            ...
            for (int i = 0; i < objs.size(); i++) {
                ...
                if (amount != 0) {
                    if (oldR == TextDescriptor.Rotation.ROT0) {
                        oldR = TextDescriptor.Rotation.ROT90;
                    } else if (oldR == TextDescriptor.Rotation.ROT90) {
                        oldR = TextDescriptor.Rotation.ROT180;
                    } else if (oldR == TextDescriptor.Rotation.ROT180) {
                        oldR = TextDescriptor.Rotation.ROT270;
                    } else {
    ...
    /**
     * Method to rotate the text on a node after the node has been
     * rotated/flipped.
     *
     * @param ni the NodeInst that was modified
     * @param dOrient the change in Orientation.
     * @param wasMirroredInX true if the node was mirrored in X before the
     * change.
     * @param wasMirroredInY true if the node was mirrored in Y before the
     * change.
     */
     ...
     * @param wasSameMirror true if the L-R mirroring was the same as the U-D
     * mirroring.
     */
...
//				origAngle += 1800;
        ...
        public boolean doIt() throws JobException {
            ...
            for (Geometric geom : list) {
                ...
                if (points != null) {
                    ...
                    for (int i = 0; i < points.length; i++) {
                        ...
                        if (oldX != newPoint.getX() || oldY != newPoint.getY()) {
                            changed = true;
                        }
                ...
                for (Iterator<PortInst> pIt = ni.getPortInsts(); pIt.hasNext();) {
                    ...
                    if (firstPort) {
                        ...
                        if (portXOffset != pXO || portYOffset != pYO) {
                            mixedportpos = true;
                        }
                    }
                }
                    ...
                    if (!ni.isCellInstance()) {
                        ...
                        for (int j = 0; j < polyList.length; j++) {
                            ...
                            if (polyPoint1.getX() == bounds.getMinX()
                                    && polyPoint2.getX() == bounds.getMaxX()) {
                                bodyXOffset = 0;
                            }
                            if (polyPoint1.getY() == bounds.getMinY()
                                    && polyPoint2.getY() == bounds.getMaxY()) {
                                bodyYOffset = 0;
                            }
                            if (bodyXOffset == 0 && bodyYOffset == 0) {
                                break;
                ...
                if (bodyXOffset != 0 || bodyYOffset != 0) {
                    ...
                    for (Iterator<Connection> aIt = ni.getConnections(); aIt.hasNext();) {
                        ...
                        if ((constr.intValue() & 1) != 0) {
                            ai.setRigid(true);
                        }
                        if ((constr.intValue() & 2) != 0) {
            ...
            for (Geometric geom : list) {
                ...
                if (!ai.headStillInPort(arcHead, false)) {
                    if (!ai.headStillInPort(origHead, false)) {
                        continue;
                    }
                    headXOff = headYOff = 0;
                }
                if (!ai.tailStillInPort(arcTail, false)) {
                    if (!ai.tailStillInPort(origTail, false)) {
                        continue;
                    }
                    tailXOff = tailYOff = 0;
                }
        ...
        public boolean doIt() throws JobException {
            ...
            for (ArcInst ai : list) {
                ...
                if (ai.getHeadLocation().getX() == ai.getTailLocation().getX()) {
                    ...
                    if (lowX < ai.getParent().getBounds().getCenterX()) {
                        lowX += stublen;
                    } else {
                        lowX -= stublen;
                    }
                    ...
                    if (lowY < ai.getParent().getBounds().getCenterY()) {
                        lowY += stublen;
                    } else {
                        lowY -= stublen;
                    }
            ...
            if (numNodeTextDeleted != 0 || numArcTextDeleted != 0 || numExportsDeleted != 0 || numVariablesDeleted != 0) {
                ...
                if (numArcTextDeleted != 0) {
                    ...
                    if (numNodeTextDeleted == 1) {
                        ...
                        msg += " text on " + numNodeTextDeleted + " Nodes";
                    }
                }
                if (numArcTextDeleted != 0) {
                    if (msg.length() == 0) {
                    ...
                    if (numArcTextDeleted == 1) {
                        ...
                        msg += " text on " + numArcTextDeleted + " Arcs";
                    }
                }
                ...
                if (numVariablesDeleted != 0) {
                    ...
                    if (numVariablesDeleted == 1) {
                        ...
                        msg += " " + numVariablesDeleted + " Variables";
                    }
                }
    ...
    public static class DeleteSelectedGeometry extends Job {
        ...
        public boolean doIt() throws JobException {
            ...
            for (ArcInst ai : arcsInCell) {
                ...
                if (DBMath.clipLine(tailPtAdj, headPtAdj, lXExt, hXExt, lYExt, hYExt)) {
                    continue;
                }
                ...
                if (!ni.isCellInstance()) {
                    if (ni.getProto().getFunction() == PrimitiveNode.Function.NODE) {
                        ...
                        if (pointBounds.getMinX() > hX || pointBounds.getMaxX() < lX
                                || pointBounds.getMinY() > hY || pointBounds.getMaxY() < lY) {
                            continue;
                        }
     ...
     * @param reconstructArcsAndExports true to reconstruct arcs to deleted cell
     * instances.
     * @param stuffToHighlight a set of objects to select (arcs and exports) if
        ...
        if (reconstructArcsAndExports) {
            for (NodeInst ni : nodesToDelete) {
                ...
                for (Iterator<Connection> cIt = ni.getConnections(); cIt.hasNext();) {
                    ...
                    if (nodesToDelete.contains(otherNi)) {
                        continue;
                    }
...
//			if (reconstructArcsAndExports && ni.isCellInstance()) continue;
    ...
    public static class CleanupChanges extends Job {
        ...
        public boolean doIt() throws JobException {
            ...
            if (!justThis) {
                infstr.append("Cell " + cell.describe(true) + ":");
            }
            boolean spoke = false;
    ...
    public static class ManyMove extends Job {
        ...
        public boolean doIt() throws JobException {
            ...
            if (highlightedObjs.size() + highlightedText.size() == 0) {
                return false;
            }
            ...
            if (highlightedObjs.size() == 1 && highlightedText.size() == 0) {
                ...
                if (firstEObj instanceof NodeInst || firstEObj instanceof PortInst) {
                    ...
                    ni.move(dX, dY);
                    if (verbose) {
                        System.out.println("Moved node " + ni.describe(false) + " by (" + dX + "," + dY + ")");
                    }
                    ...
                    if (ai.getHeadLocation().getX() != ai.getTailLocation().getX()
                        ...
                        if (ai.isFixedAngle() && !ai.isRigid()) {
                            ...
                            for (j = 0; j < 2; j++) {
                                ...
                                if (highlightedObjs.contains(oai)) {
                                    break;
                                }
                            }
                            if (j >= 2) {
            ...
            if (found) {
                ...
                if (verbose) {
                    System.out.println("Moved multiple objects by (" + TextUtils.formatDistance(dX)
            ...
            for (ElectricObject eobj : highlightedObjs) {
                if (eobj instanceof ArcInst) {
                    ...
                    if (ai.isSlidable()) {
                        ...
                        if (ai.headStillInPort(newHead, true) && ai.tailStillInPort(newTail, true)) {
                            continue;
                        }
                    }
                }
            ...
            if (foundArc && onlySlidable) {
                for (ElectricObject eobj : highlightedObjs) {
                    if (eobj instanceof ArcInst) {
                        ...
                        if (verbose) {
                            System.out.println("Moved arc " + ai.describe(false) + " by (" + dX + "," + dY + ")");
                        }
                    }
                }
                updateStatusBar = true;
            ...
            for (ElectricObject eobj : highlightedObjs) {
                ...
                if (!headInPort && !tailInPort) {
                    for (int k = 0; k < 2; k++) {
                        NodeInst ni;
                        if (k == 0) {
                            ni = ai.getHeadPortInst().getNodeInst();
                        } else {
                            ni = ai.getTailPortInst().getNodeInst();
                        }
                        Point2D nPt = nodeLocation.get(ni);
                        if (ni.getAnchorCenterX() != nPt.getX() || ni.getAnchorCenterY() != nPt.getY()) {
                            continue;
                        }
        ...
        /**
         * Method to move the "numtexts" text objects described (as highlight
         * strings) in the array "textlist", by "odx" and "ody". Geometry
         * objects in "list" (NOGEOM-terminated) and the "total" nodes in
         * "nodelist" have already been moved, so don't move any text that is on
         * these objects.
         */
...
//            System.out.println("Moving cell center to (" + ctr.getX() + "," + ctr.getY() + ")");
    ...
    /**
     * Store information of new arc to be created that reconnects two arcs that
     * will be deleted
     */
    private static class ReconnectedArc implements Serializable {

        /**
         * port at other end of arc
         */
        private PortInst[] reconPi;
        /**
         * coordinate at other end of arc
         */
        private EPoint[] recon;
        /**
         * old arc instances that will be deleted
         */
        private ArcInst[] reconAr;
        /**
         * prototype of new arc
         */
        private ArcProto ap;
        /**
         * width of new arc
        ...
        private double wid;
        /**
         * true to make new arc have arrow on head
         */
        private boolean directionalHead;
        /**
         * true to make new arc have arrow on tail
         */
        private boolean directionalTail;
        /**
         * true to make new arc have arrow on body
         */
        private boolean directionalBody;
        /**
         * true to extend the head of the new arc
         */
        private boolean extendHead;
        /**
         * true to extend the tail of the new arc
         */
        private boolean extendTail;
        /**
         * true to negate the head of the new arc
         */
        private boolean negateHead;
        /**
         * true to negate the tail of the new arc
         */
        private boolean negateTail;
        /**
         * the name to use on the reconnected arc
         */
        private String arcName;
        /**
         * TextDescriptor for the reconnected arc name
         */
    ...
    /**
     * This class handles deleting pins that are between two arcs, and
     * reconnecting the arcs without the pin.
     */
    public static class Reconnect implements Serializable {
         ...
         * @param allowdiffs true to allow differences in the two arcs. If this
         * is false, then different width arcs, or arcs that are not lined up
         * precisely, will not be considered for reconnection.
         * @param checkPermission true to check that the node can be changed.
         * @param ep EditingPreferences
         * @return a Reconnect object that describes the reconnection to be
         * done. Returns null if no reconnection can be found.
         */
        public static Reconnect erasePassThru(NodeInst ni, boolean allowdiffs, boolean checkPermission, EditingPreferences ep) {
            ...
            if (checkPermission && cantEdit(cell, ni, true, false, true) != 0) {
                return null;
            }
        ...
        private static ReconnectedArc reconnectArcs(PortInst pi, ArcInst ai1, ArcInst ai2, boolean allowdiffs) {
            ...
            if (!allowdiffs) {
                ...
                if (delta[1].getX() == 0 && delta[1].getY() == 0) {
                    zeroLength = true;
                }
                if (!zeroLength && delta[0].getX() * delta[1].getX() + delta[0].getY() * delta[1].getY() >= 0) {
                    return null;
                }
        ...
        public List<ArcInst> reconnectArcs(EditingPreferences ep) {
            ...
            for (ReconnectedArc ra : reconnectedArcs) {
                if (!ra.reconPi[0].getNodeInst().isLinked() || !ra.reconPi[1].getNodeInst().isLinked()) {
                    continue;
                }
     ...
     * @param direction the direction to spread: 'u' for up, 'd' for down, 'l'
     * for left, 'r' for right.
     ...
     * @param geomSeen the HashSet used to mark nodes that are examined. This is
     * called from "spread" to propagate along Manhattan arcs that are in the
     * correct orientation (along the spread line).
     */
    ...
    /**
     * Method to replace node "oldNi" with a new one of type "newNp" and return
     * the new node. Also removes any node-specific variables.
     */
     ...
     * @param allowMissingPorts true to allow replacement to have missing ports
     * and, therefore, delete the arcs that used to be there.
     * @param ep EditingPreferences
...
//        inheritAttributes(newNi, ep);
    ...
    public static void inheritAttributes(NodeInst ni, EditingPreferences ep) {
        ...
        for (Iterator<Variable> it = cell.getVariables(); it.hasNext();) {
            Variable var = it.next();
            if (!var.getTextDescriptor().isInherit()) {
                continue;
            }
    ...
    /**
     * Method to add all inheritable export variables from export "pp" on cell
     * "np" to instance "ni".
     */
     ...
     * Normally, it is simply "var.getObject()", but if it is a string with the
     * "++" or "--" sequence in it, then it indicates an
     * auto-increments/decrements of that numeric value. The returned object has
     * the "++"/"--" removed, and the original variable is modified.
     *
    ...
    private static Object inheritAddress(ElectricObject addr, Variable var, EditingPreferences ep) {
        ...
        int i = incrPoint - 1;
        for (; i >= 0; i--) {
            if (!TextUtils.isDigit(str.charAt(i))) {
                break;
            }
        }
        i++;
        int curVal = TextUtils.atoi(str.substring(i));
        if (str.charAt(incrPoint) == '+') {
            curVal++;
        } else {
            curVal--;
        }
...
//	}
     ...
     */
    /**
     * Method to test whether a NodeInst can be modified in a cell. Throws an
     * exception if not.
     *
     * @param cell the Cell in which the NodeInst resides.
     * @param item the NodeInst (may be null).
     * @param giveError true to print an error message if the modification is
     * disallowed.
     * @param allowInstanceChange true to allow a change to an instance when
     * instances are locked (this allows exports to be created and deleted).
     */
    ...
    /**
     * Method to tell whether a NodeInst can be modified in a cell. WARNING:
     * method may change the database if the user disables a cell lock, so
     * method must be called inside of a Change job.
     *
     * @param cell the Cell in which the NodeInst resides.
     * @param item the NodeInst (may be null).
     * @param giveError true to print an error message if the modification is
     ...
     * @return positive if the edit CANNOT be done. Return negative if the edit
     * CANNOT be done and the overall operation should be canceled. Return zero
     * if the operation CAN be done.
     */
    public static int cantEdit(Cell cell, NodeInst item, boolean giveError, boolean allowInstanceChange, boolean insideJob) {
        ...
        if (item != null) {
            if (item.isLocked()) {
                ...
                if (ret == 3 || ret == -1) {
                    return -1;  // -1 represents ESC or cancel
                }
            }
            boolean complexNode = false;
            if (!item.isCellInstance()) {
