...
public class Spice extends Topology {
    ...
    /**
     * key of Variable holding Xyce templates.
     */
    public static final Variable.Key SPICE_XYCE_TEMPLATE_KEY = Variable.newKey("ATTR_SPICE_template_xyce");
    /**
    ...
    /**
     * key of deprecated Variable holding model file
     */
    ...
    /**
     * maximum subcircuit name length
     */
    ...
    /**
     * legal characters in a spice deck
     */
    ...
    /**
     * legal characters in a CDL deck
     */
    ...
    /**
     * default Technology to use.
     */
    private Technology layoutTechnology;
    /**
     * Technology of top cell.
     */
    private Technology curTech;
    /**
     * Mask shrink factor (default =1)
     */
    private double maskScale;
    /**
     * True to write CDL format
     */
    ...
    /**
     * Template Key for current spice engine
     */
    private Variable.Key preferedEngineTemplateKey;
    /**
     * Special case for HSpice for Assura
     */
    private boolean assuraHSpice = false;
    /**
     * Spice type: 2, 3, H, P, etc
     */
    ...
    /**
     * Networks exempted during parasitic ext
     */
    private SpiceExemptedNets exemptedNets;
    /**
     * Whether or not to write empty subcircuits
     */
    private boolean writeEmptySubckts = true;

    /**
     * Flat measurements file
     */
    private FlatSpiceCodeVisitor spiceCodeFlat = null;

    /**
     * map of "parameterized" cells not covered by Topology
     */
    ...
    /**
     * map of shortened instance names
     */
...
//    			String templateFile = deckPath + File.separator + cell.getName() + ".cdltemplate";
                ...
                if (out.openTextOutputStream(templateFile)) {
                    return out.finishWrite();
                }
     ...
     */
    Spice(SpicePreferences sp) {
        localPrefs = sp;
    }

    /**
     * Method called once by the traversal mechanism. Initializes Spice
     * netlisting and writes headers.
     */
    protected void start() {
        ...
        switch (spiceEngine) {
            case SPICE_ENGINE_2:
                preferedEngineTemplateKey = SPICE_2_TEMPLATE_KEY;
                break;
            case SPICE_ENGINE_3:
            ...
            case SPICE_ENGINE_XYCE:
                preferedEngineTemplateKey = SPICE_XYCE_TEMPLATE_KEY;
                break;
            case SPICE_ENGINE_O:
                break;
            case SPICE_ENGINE_H_ASSURA:
                preferedEngineTemplateKey = SPICE_A_TEMPLATE_KEY;
                assuraHSpice = true;
                break;
            case SPICE_ENGINE_H_CALIBRE:
                preferedEngineTemplateKey = SPICE_C_TEMPLATE_KEY;
                assuraHSpice = true;
                break;
        }
        if (useCDL) {
    ...
    /**
     * Method called by traversal mechanism to write one level of hierarchy in
     * the Spice netlist. This could be the top level or a subcircuit. The bulk
     * of the Spice netlisting happens here.
     *
    ...
    protected void writeCellTopology(Cell cell, String cellName, CellNetInfo cni, VarContext context, Topology.MyCellInfo info) {
                ...
                if (globalSize > 0) {
                    ...
                    for (int i = 0; i < globalSize; i++) {
                        ...
                        if (global == Global.power) {
                            if (getPowerName(null) != null) {
                                name = getPowerName(null);
                            }
                        }
                        if (global == Global.ground) {
                            if (getGroundName(null) != null) {
        ...
        if (useCDL || cell.getView() != View.LAYOUT) {
...
//                      spNet.nonDiffCapacitance += layer.getEdgeCapacitance() * perim * maskScale;
        ...
        if (cell == topCell && !useCDL && !localPrefs.writeSubcktTopCell) {
            ...
            for (Iterator<CellSignal> sIt = cni.getCellSignals(); sIt.hasNext();) {
                ...
                if (ignoreSubcktPort(cs)) {
                    continue;
                }
                if (!cs.isGlobal() && cs.getExport() == null) {
                    continue;
                }
            ...
            if (!useCDL && localPrefs.useCellParameters) {
                ...
                for (Iterator<Variable> it = cell.getParameters(); it.hasNext();) {
                    ...
                    if (DETECT_SPICE_PARAMS && !spiceParams.contains(paramVar.getKey())) {
                        continue;
                    }
...
//						continue;
                ...
                if (useCDL && SimulationTool.isCDLLashOn()) {
                    infstr.append(" /");
                } else {
                    infstr.append(" ");
                }
                ...
                if (!useCDL && localPrefs.useCellParameters) {
                    ...
                    for (Iterator<Variable> it = subCell.getParameters(); it.hasNext();) {
                        ...
                        if (!USE_JAVA_CODE && !isNetlistableParam(subCell, paramVar)) {
                            continue;
                        }
                    ...
                    if (resistVar != null) {
                        ...
                        if (fun.isComplexResistor()) {
                            ...
                            if (fun == PrimitiveNode.Function.RESPPOLY) {
                                ...
                                prepend = "rnpolywo";
                            } else if (fun == PrimitiveNode.Function.RESPPOLYM) {
                                prepend = "rppolywo_m";
                            } else if (fun == PrimitiveNode.Function.RESNPOLYM) {
                                prepend = "rnpolywo_m";
                            } else if (fun == PrimitiveNode.Function.RESPNSPOLY) {
                                prepend = "rpponsrpo";
                            } else // made up value
                            if (fun == PrimitiveNode.Function.RESNNSPOLY) {
                                prepend = "rnponsrpo";
                            } else // made up value
                            if (fun == PrimitiveNode.Function.RESPWELL) {
                                prepend = "rpwod ";
            ...
            } else if (fun == PrimitiveNode.Function.TRADMES
                    || // DMES (Depletion) transistor
                    fun == PrimitiveNode.Function.TRA4DMES) // DMES (Depletion) 4-port transistor
            {
                modelChar = "Z";
                biasCs = null;
                modelName = "DMES";
            } else if (fun == PrimitiveNode.Function.TRAEMES
                    || // EMES (Enhancement) transistor
            ...
            if (ni.getName() != null) {
                modelChar += getSafeNetName(ni.getName(), false);
            }
            StringBuffer infstr = new StringBuffer();
                    ...
                    if (size.getDoubleLength() > 0) {
                        ...
                        if (fun == PrimitiveNode.Function.TRANMOS || fun == PrimitiveNode.Function.TRA4NMOS
                            ...
                            if (size.getDoubleLength() == 0 && size.getLength() instanceof String) {
                                ...
                                if (!localPrefs.writeTransSizeInLambda && !st090laytrans) {
                                    infstr.append("U");
                                }
                            }
                        }
            ...
            if (spNetGate == null || spNetSource == null || spNetDrain == null) {
                continue;
            }
            ...
            if (spLevel == SimulationTool.SpiceParasitics.RC_PROXIMITY) {
                ...
                for (SpiceSegmentedNets.NetInfo netInfo : segmentedNets.getUniqueSegments()) {
                    if (netInfo.getCap() > cell.getTechnology().getMinCapacitance()) {
                        if (netInfo.getName().equals("gnd")) {
                            continue;           // don't write out caps from gnd to gnd
                        }
     ...
     */
    /**
     * Method to create a parameterized name for nodable. If the node is not
     * parameterized, returns zero. If it returns a name, that name must be
...
//              for(Iterator<Variable> it = no.getDefinedParameters(); it.hasNext(); )
                for (Iterator<Variable> it = no.getParameters(); it.hasNext();) {
                    ...
                    if (DETECT_SPICE_PARAMS && !spiceParams.contains(var.getKey())) {
                        continue;
                    }
                    if (USE_JAVA_CODE) {
                        if (useCellParams && !spiceParams.contains(null)) {
                            continue;
                        }
                    } else {
                        if (useCellParams && !var.isJava()) {
                            continue;
                        }
                    }
                    paramValues.add(var);
...
//        }
    ...
    /**
     * Method to determine if the cell name must be truncated due to character
     * limit. If yes, function will remove the first characters of the name
     * string as variations of the same cell would differ in the last
     * characters.
     *
...
//            uniqueCellName = uniqueCellName.delete(limit-10, uniqueCellName.length());
    ...
    /**
     * Method to return the CodeExpression on a Variable. Prints warnings if
    ...
    /**
     * Returns true if variable can be netlisted as a spice parameter
     * (unevaluated expression). If not, it should be evaluated before
     * netlisting.
     *
    ...
    private boolean isNetlistableParam(ElectricObject eObj, Variable var) {
        if (USE_JAVA_CODE) {
            ...
            if (ce.getHSpiceText(localPrefs.unitsPrecision, false) != null) {
                return true;
            }
            return false;
        }
    ...
    /**
     * Returns text representation of instance parameter on specified Nodable in
     * specified VarContext The text representation can be either spice
     * parameter expression or a value evaluated in specified VarContext
     *
     ...
     * @param instParam instance parameter
     * @param forceEval true to always evaluate parameter
     * @param wrapped true if the string is already "wrapped" (with parenthesis)
     * and does not need to have quotes around it.
     * @return true if the variable should be netlisted as a spice parameter
     */
    private String evalParam(VarContext context, Nodable no, Variable instParam, boolean forceEval, boolean inPars, boolean wrapped) {
        ...
        System.out.println("000" + String.valueOf(obj));
        if (obj == null) {
...
//                    printlnParams(cell + " collected params", params);
    ...
    /**
     * Method to tell which Variables are important for primitive node in this
     * netlister
     *
     * @param pn primitive node to tell
     * @return a set of important variables or null if all variables may be
     * important
     */
    ...
    /**
     * Method to determine which Spice engine is being targeted and to get the
     * proper template variable. Looks in associated icon/schematics cells to
     * find it.
     *
     ...
     * @param cni the cell net info of cell in which the nodable exists (if
...
//            	nodeName = nodeName.replaceAll("[\\[\\]]", "_");
                ...
                System.out.println("2" + paramName);
                ...
                if (varKey != null) {
                    ...
                    System.out.println("21" + attrVar.toString());
                }
                if (attrVar == null) {
                    infstr.append("??");
                } else {
                    ...
                    System.out.println("22" + parentVar.toString());
                    if (USE_JAVA_CODE) {
                        pVal = evalParam(context, no, attrVar, forceEval, true, infstr.inParens());
                        System.out.println("26 " + pVal);
                        if (infstr.inQuotes()) {
                            pVal = trimSingleQuotes(pVal);
                        }
                        System.out.println("23" + pVal);
                    } else {
                        if (!useCDL && localPrefs.useCellParameters
...
//                    if (attrVar.getCode() != TextDescriptor.Code.NONE)
    ...
    /**
     * Find variables in 'line'. Ports and variables should be referenced via
     * $(name)
     *
     * @param line the string to search and replace within
    ...
    private void findVarsInLine(String line, NodeProto prototype, boolean isPortReplacement, Set<Variable.Key> vars) {
        ...
        for (int pt = 0; pt < line.length(); pt++) {
            ...
            if (paramName.equalsIgnoreCase("node_name") && prototype != null) {
                continue;
            }
    ...
    /**
     * Get the global associated with this net. Returns null if net is not a
     * global.
     *
     * @param net
    ...
    /**
     * Method to report that export names do NOT take precedence over arc names
     * when determining the name of the network.
     */
    ...
    /**
     * Method to report that library names are NOT always prepended to cell
     * names.
     */
    ...
    /**
     * Method to decide whether signal sorting supports ascending/descending
     * order
     */
    protected boolean doesSupportDescAsc() {
        return localPrefs.supportAscDescOrdering;
    }

    /**
     * Method to adjust a network name to be safe for Spice output. Spice has a
     * list of legal punctuation characters that it allows.
     */
    ...
    /**
     * Method called during hierarchy traversal. Called at the end of the enter
     * cell phase.
     */
    ...
    /**
     * Method to report that not to choose best export name among exports
     * connected to signal.
     */
     ...
     * subcells, override this method. If this cell has a spice template, skip
     * it
     */
    ...
    /**
     * Since the Spice netlister should write a separate copy of schematic cells
     * for each icon, this override returns true.
     */
    ...
    /**
     * Method called when a cell is skipped. Performs any checking to validate
...
//    	}
     ...
     *
     * @param context the context of the nodable for finding M-factors farther
     * up the hierarchy.
    ...
    /**
     * write a header for "cell" to spice deck "sim_spice_file" The model cards
     * come from a file specified by tech:~.SIM_spice_model_file or else
     * tech:~.SIM_spice_header_levelN The spice model file can be located in
     * el_libdir
     */
...
//        multiLinePrint(false, ".OPTIONS NOMOD NOPAGE\n");
        ...
        switch (level) {
            case 1:
                header = layoutTechnology.getSpiceHeaderLevel1();
                break;
            case 2:
                header = layoutTechnology.getSpiceHeaderLevel2();
                break;
            case 3:
                header = layoutTechnology.getSpiceHeaderLevel3();
                break;
        }
        if (header != null) {
    ...
    /**
     * Write a trailer from an external file, defined as a variable on the
     * current technology in this library: tech:~.SIM_spice_trailer_file if it
     * is available.
     */
    ...
    /**
     * Function to write a two port device to the file. Complain about any
     * missing connections. Determine the port connections from the exports in
     * the instance prototype. Get the part number from the 'part' number value;
     * increment it. The type of device is declared in type; extra is the string
     * data acquired before calling here. If the device is connected to the same
     * net at both ends, do not write it. Is this OK?
     */
    private void writeTwoPort(NodeInst ni, String partName, String extra, CellNetInfo cni, Netlist netList,
        ...
        if (ni.getName() != null) {
            partName += getSafeNetName(ni.getName(), false);
        }
     ...
     * associated with a NodeInst. If the node is mult_layer, then determine the
     * dominant capacitance layer, and add its area; all other layers will be
     * added as well to the extra_area total. Continue out of the ports on a
     * complex cell
     */
    private void addNodeInformation(Netlist netList, Map<Network, SpiceNet> spiceNets, NodeInst ni) {
        ...
        if (ni.isCellInstance()) {
            return;  // No area for complex nodes
        }
        ...
        for (int i = 0; i < tot; i++) {
            ...
            if (layer.getTechnology() != curTech) {
                continue;
            }
     ...
     * it will contribute to the area of sources and drains, and the other
     * layers will be ignored. This is not quite the same as the rule used for
     * contact (node) structures. Note: the earlier version of this function
     * assumed that diffusion arcs would always have zero capacitance values for
     * the other layers; this produces an error if any of these layers have
     * non-zero values assigned for other reasons. So we will check for the
    ...
    /**
     * Finds cells that must be uniquified during netlisting. These are cells
     * that have parameters which are Java Code. Because the Java Code
     ...
     * @return true if the cell has been marked to unquify (which is true if any
     * cell below it has been marked).
     */
    private boolean markCellsToUniquify(Cell cell) {
        if (uniquifyCells.containsKey(cell)) {
            return uniquifyCells.get(cell) != null;
        }
        boolean mark = false;
        ...
        for (Iterator<NodeInst> it = cell.getNodes(); it.hasNext();) {
            ...
            for (Variable.Key key : varkeys) {
                ...
                if (var != null && !isNetlistableParam(ni, var)) {
                    mark = true;
                }
            }
            if (!ni.isCellInstance()) {
            ...
            if (getEngineTemplate(proto) != null) {
                continue;
            }
            if (markCellsToUniquify(proto)) {
                mark = true;
            }
        }
    ...
    public boolean cellIsEmpty(Cell cell) {
        ...
        for (Iterator<NodeInst> it = cell.getNodes(); it.hasNext();) {
            ...
            if (fun.isResistor()
                    ...
                    fun == PrimitiveNode.Function.INDUCT
                    || fun.isCapacitor()
                    || // == PrimitiveNode.Function.CAPAC || fun == PrimitiveNode.Function.ECAPAC ||
                    fun == PrimitiveNode.Function.DIODE || fun == PrimitiveNode.Function.DIODEZ) {
                empty = false;
                break;
            }
    ...
    /**
     * Method to determine the name to use for a CellSignal. It may use the
     * export name if explicit ports are being written to each subcircuit.
     *
    ...
    private static String getSafeNetName(String name, String legalSpiceChars, SimulationTool.SpiceEngine spiceEngine) {
        ...
        if (TextUtils.isDigit(name.charAt(0))
                && spiceEngine != SimulationTool.SpiceEngine.SPICE_ENGINE_G
                && spiceEngine != SimulationTool.SpiceEngine.SPICE_ENGINE_P
                && spiceEngine != SimulationTool.SpiceEngine.SPICE_ENGINE_2) {
            sb.append('_');
        }
    ...
    private String formatParam(String param, TextDescriptor.Unit u, boolean wrapped) {
        ...
        if (TextUtils.isANumberPostFix(value)) {
            return value;
        }
        ...
        if (!wrapped) {
            ...
            if (spiceEngine == SimulationTool.SpiceEngine.SPICE_ENGINE_2
                    || spiceEngine == SimulationTool.SpiceEngine.SPICE_ENGINE_3) {
                return value;
            }
    ...
    /**
     * Method to report an error or warning message that is built in the
     * infinite string. The message is sent to the messages window, errorLogger
     * and also to the SPICE deck "f".
     */
    ...
    /**
     * Formatted output to file "stream". All spice output is in upper case. The
     * buffer can contain no more than 1024 chars including the
     * newlinelastMoveTo and null characters. Doesn't return anything.
     */
    ...
    private static class SpiceNet {
        ...
        /**
         * merged geometry for this network
         */
        PolyMerge merge;
        /**
         * area of diffusion
...
//		/** amount of capacitance in non-diffusion */	float     nonDiffCapacitance = 0;
        /**
         * number of transistors on the net
         */
        int transistorCount;
