...
public class Inv2i {
	...
	public static Cell makePart(double sz, StdCellParams stdCell) {
		...
		TrackRouter pGates = new TrackRouterH(tech.m1(), 3, pGatesY, tech, ep, buf);
		for (int i=0; i<pmos.nbGates(); i++) {
			pGates.connect(pmos.getGate(i, 'B'));
		}
		TrackRouter nGates = new TrackRouterH(tech.m1(), 3, nGatesY, tech, ep, buf);
		for (int i=0; i<nmos.nbGates(); i++) {
			nGates.connect(nmos.getGate(i, 'T'));
		}
		
		// input n
		LayoutLib.newExport(buf, "in[n]", ep, PortCharacteristic.IN, tech.m1(),
							4, inNX, nGatesY);
		...
		LayoutLib.newExport(buf,"in[p]", ep, PortCharacteristic.IN, tech.m1(),
							4, inPX, pGatesY);
		pGates.connect(buf.findExport("in[p]"));
		
		// Buf output
		double outX = inPX + 2 + 3 + 2;	// m1_wid/2 + m1_sp + m1_wid/2
		LayoutLib.newExport(buf, "out", ep, PortCharacteristic.OUT, tech.m1(),
							4, outX, 0);
		TrackRouter outHi = new TrackRouterH(tech.m2(), 4, outHiY, tech, ep, buf);
		outHi.connect(buf.findExport("out"));
		for (int i=1; i<pmos.nbSrcDrns(); i+=2) {
			outHi.connect(pmos.getSrcDrn(i));
		}
		
		TrackRouter outLo = new TrackRouterH(tech.m2(), 4, outLoY, tech, ep, buf);
