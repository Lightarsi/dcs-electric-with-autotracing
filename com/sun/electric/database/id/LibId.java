...
public final class LibId implements Serializable {
    ...
    /**
     * Checks if character is not valid as part of a library name
     * @param ch character to check. Invalid characters are replaced by '-'.
     ...
     *   or null if specified name is null or empty.
     */
    public static String legalLibraryName(String libName) {
        ...
        for (; i < libName.length(); i++) {
            if (isInvalidLibraryCharacter(libName.charAt(i))) {
                break;
            }
        }
        if (i == libName.length()) {
        ...
        for (; i < libName.length(); i++) {
            char ch = chars[i];
            chars[i] = isInvalidLibraryCharacter(ch) ? '-' : ch;
        }
